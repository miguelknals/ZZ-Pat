...
Class ClaseReceiveF ' con public class comparto las definiciones
    Dim nl As String = Environment.NewLine
    Dim ListaCarpetasTraducir As New ClaseListaCarpetasTraducir
    Dim ListaMFTbyTranslator As New ClaseListaMFTbyTranslator
                ...
                Select Case TipoDestino
                    Case "SHARED_DRIVE"
                        ...
                        Try
                            ...
                            For Each fila In ListaCarpetasTraducir.tCarpetasTraducir.Rows
                                ...
                                AnyadetxtSalida("Done!" & nl)
                            Next
                            ' también debo borrar el MTF file
                            ' archivo MFT en el repositorio
                            source = destino.DestSharedDriveNombre & "\" & Path.GetFileName(ArchivoMF)
                    ...
                    Case "PATH"
                        ...
                        Try
                            ...
                            ' también debo borrar el MTF file
                            ' archivo MFT en el repositorio
                            source = destino.DestPath & "\" & Path.GetFileName(ArchivoMF)
                            If File.Exists(source) Then
                                File.Delete(source)
                            End If
...
continua_sin_traductor:
            ...
            Else
                ...
                If bFinCal = False Then ' si es true no hago nada, si es false, tenog que calcularlo todo
                    ...
                    If reemplazarcarpetas Then
                        ...
                        Dim DirectorioCopiaSeguridadSinUnidad As String = _
                        ...
                        Try
                            ...
                            mandato = String.Format(mandato, carpeta, unidad, DirectorioCopiaSeguridadSinUnidad)
                            'AnyadeSalida(mdto)
                            ejecuta(mandato, rc)
                            If rc <> 0 Then
                                ' debe existir, si no puedo, me rindo
                                MsgBox("Folder to be updtated in OTM2 cannot be exported as backup in the backup directory " _
                                       & DirectorioCopiaSeguridad & nl & _
                                     " Maybe there the folder already has been exported in the backup directory. " & _
                                     " RC=999 Indicates a problem in the shell environment.  " & _
                                     "RC=" & rc.ToString, MsgBoxStyle.Critical, _
                            ...
                            mandato = String.Format(mandato, carpeta, TargetDirectorioSinUnidad, unidad)
                            ' AnyadeSalida(mdto)
                            ejecuta(mandato, rc)
                            If rc <> 0 Then
                                MsgBox(String.Format("Cannot import  updating folder {0} {1} . RC={2}", _
                            ...
                            ArchivoFinCal = TargetDirectorioSinUnidad & String.Format("\{0}_{1}_fin_cal.rpt", carpeta, row("Perfil"))
    ...
    Private Sub btnSearchByTranslator_Click(sender As Object, e As RoutedEventArgs) Handles btnSearchByTranslator.Click

        ' la clase con la información 
        Dim VP As New DialogSearchMFTbyTranslator
        VP.ShowDialog()
        Dim MTFFileByTranslator As String = VP.RetreiveMFTFromTranslator
        If MTFFileByTranslator <> "" Then
            txtArchivoMF.Text = MTFFileByTranslator
        End If
        ' debo cargar los parámetros
        ' me debe devolver un path
    End Sub
